...
if __name__ == '__main__':
    ...
    password = '123456'  # 密码
    port = 21  # 端口号
    rootdir_local = 'F:/ls/xml/' + datenow # 本地目录
    rootdir_remote = '/'  # 远程目录
    download_file_arr = ['35919_3353520_mp.xml.gz',
                         '39673_3353520_mp.xml.gz',
                         '40224_3353520_mp.xml.gz',
                         '40322_3353520_mp.xml.gz',
                         '40798_3353520_mp.xml.gz',
                         '41770_3353520_mp.xml.gz',
                         '42382_3353520_mp.xml.gz']

    if not os.path.exists(rootdir_local):
        os.mkdir(rootdir_local)

    f = MYFTP(hostaddr, username, password, rootdir_remote, port)
    f.login()
    # f.download_files(rootdir_local, rootdir_remote)
    for download_file in download_file_arr:
        local_file = os.path.join(rootdir_local, download_file)
        remote_file = os.path.join(rootdir_remote, download_file)
        f.download_file(localfile=local_file, remotefile=remote_file)
    timenow = time.localtime()
    datenow = time.strftime('%Y-%m-%d', timenow)
    logstr = u"%s 成功下载文件列表 \n" % datenow
    debug_print(logstr)